...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 14 22:19:31 2019
...
from app.strategies.turtle import TurtleStrategy, TurtlePortfolio


DAILY_DB_NAME = 'turtledata'
...
class DailyResult(object):
    ...
    def updateBar(self, bar):
        """更新K线"""
        self.closeDict[bar.vt_symbol] = bar.close
...
class BacktestingEngine(object):
    ...
    def initPortfolio(self, filename, portfolioValue=10000000):
        ...
        print(u'投资组合的初始价值%s' %(portfolioValue))
    ...
    def loadData(self):
        ...
        for vtSymbol in self.vtSymbolList:
            ...
            for d in cursor:
                ...
                bar.low= d['low']
                bar.close = d['close']
                bar.vt_symbol = d['vtSymbol']
                
                barDict = self.dataDict.setdefault(bar.datetime, OrderedDict())
                
                barDict[bar.vt_symbol] = bar
           
            print(u'%s数据加载完成，总数据量：%s' %(vtSymbol, cursor.count()))
    ...
    def runBacktesting(self):
        ...
        for dt, barDict in self.dataDict.items():
            ...
            self.result.updatePos(self.portfolio.posDict)  #更新仓位
    ...
    def showResult(self):
        ...
        # 输出统计结果
        print('-' * 30)
        print(u'首个交易日：\t%s' % result['startDate'])
        print(u'最后交易日：\t%s' % result['endDate'])
        
        print(u'总交易日：\t%s' % result['totalDays'])
        print(u'盈利交易日\t%s' % result['profitDays'])
        print(u'亏损交易日：\t%s' % result['lossDays'])
        
        print(u'起始资金：\t%s' % self.portfolioValue)
        print(u'结束资金：\t%s' % formatNumber(result['endBalance']))
    
        print(u'总收益率：\t%s%%' % formatNumber(result['totalReturn']))
        print(u'年化收益：\t%s%%' % formatNumber(result['annualizedReturn']))
        print(u'总盈亏：\t%s' % formatNumber(result['totalNetPnl']))
        print(u'最大回撤: \t%s' % formatNumber(result['maxDrawdown']))   
        print(u'百分比最大回撤: %s%%' % formatNumber(result['maxDdPercent']))   
        
        print(u'总手续费：\t%s' % formatNumber(result['totalCommission']))
        print(u'总滑点：\t%s' % formatNumber(result['totalSlippage']))
        print(u'总成交笔数：\t%s' % formatNumber(result['totalTradeCount']))
        
        print(u'日均盈亏：\t%s' % formatNumber(result['dailyNetPnl']))
        print(u'日均手续费：\t%s' % formatNumber(result['dailyCommission']))
        print(u'日均滑点：\t%s' % formatNumber(result['dailySlippage']))
        print(u'日均成交笔数：\t%s' % formatNumber(result['dailyTradeCount']))
        
        print(u'日均收益率：\t%s%%' % formatNumber(result['dailyReturn']))
        print('收益标准差：\t%s%%' % formatNumber(result['returnStd']))
        print(u'Sharpe Ratio：\t%s' % formatNumber(result['sharpeRatio']))
